...
import org.d3if0105.foodiefiends.navigation.Screen

import org.d3if0105.foodiefiends.ui.theme.FoodieFiendsTheme
import org.d3if0105.foodiefiends.util.SettingsDataStore
...
fun ScreenContent(showList: Boolean, modifier: Modifier, navController: NavHostController) {
    ...
    if (data.isEmpty()) {
        Column (
            ...
            val image = painterResource(id = R.drawable.flying)
...
@Composable
fun ListItem(foodie: Foodie, onClick:()-> Unit) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() }
        ...
        Text(
            text = foodie.namaMenu,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
            fontWeight = FontWeight.Bold
        )
        Text(
            text = foodie.kategori,
            maxLines = 2,
            overflow = TextOverflow.Ellipsis
        )
        Text(
            text = foodie.deskripsi,
            maxLines = 3,
            overflow = TextOverflow.Ellipsis
        )
    }
...
@Composable
fun GridItem(Foodie: Foodie, onClick: () -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() },
